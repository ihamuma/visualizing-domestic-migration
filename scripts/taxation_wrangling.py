...
unmatching_columns = dataframe_helper.check_dataframes_for_unidentical_columns(dict_of_dfs=dfs, reference_df_key='taxation_2011_df')

assert unmatching_columns['dataframes_with_unmatching_columns'] == {}

all_taxation_years_df = pd.concat(dfs, ignore_index=True)
all_taxation_years_df = all_taxation_years_df.sort_values(by=['year', 'municipality'], ascending=[True, True])
...
columns_to_float = ['taxable_income', 
                    ...
                    'tax_return',
                    'residual_tax']
all_taxation_years_df = dataframe_helper.convert_columns_to_float(all_taxation_years_df, columns_to_float)
