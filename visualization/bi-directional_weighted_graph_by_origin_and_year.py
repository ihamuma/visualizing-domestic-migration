...
migration_df = pd.read_parquet(
    f'processed_data/directional_graph_{origins[0]}-{origins[1]}_{year}.parquet')
...
plt.title(
    f"Inter-regional migration by persons of foreign origin {year}", fontproperties=prop)
plt.savefig(
    f'visualization/results/Overlayed Weighted Graph Migration {origins[0]}-{origins[1]} {year}.png', dpi=300)
