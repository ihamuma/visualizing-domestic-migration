...
def verify_and_create_files():
    ...
    if not os.path.isfile(migration_file_path):
        ...
        migration_df = get_migration_data(
            migration_url, migration_query, migration_ages)
        migration_df = fix_dataframe_dtypes(migration_df)
        migration_df = migration_df[migration_df['Region of arrival']
                                    != migration_df['Region of departure']].reset_index(drop=True)
    ...
    if not os.path.isfile(employment_file_path):
        ...
        print(
            f'Employment data successfully written to {employment_file_path}')
    else:
